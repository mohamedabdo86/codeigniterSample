...
$this_table_name = "section_slidesshow";
...
$pictures_columns[0] = "section_slidesshow_image_ar";
$pictures_columns[1] = "section_slidesshow_image";
...
$form_array = array
(
	array("database_name" => "section_slidesshow_section_id" , "title" => "Section" , "type" => "select" , "required" => "1" ,"attached_options" => $display_section ,"current_value"=> $display['section_slidesshow_section_id'] ) ,
	...
	array("database_name" => "section_slidesshow_flag" , "title" => "Select Type Of URL" , "type" => "select" , "required" => "1" , "message" => "If an default Nestle URL EX. best_cook/ask_an_expert<br/> Else If an External URL EX. https://www.facebook.com/", "attached_options" => $display_url, "current_value"=> $display['section_slidesshow_flag']) ,
	array("database_name" => "section_slidesshow_url" , "title" => "URL" , "type" => "text" , "required" => "0" , "message" => "EX. best_cook/ask_an_expert"  , "current_value"=> $display['section_slidesshow_url'] ) ,
	
	array("database_name" => "section_slidesshow_active" , "title" => "Active" , "type" => "select" , "required" => "0" , "current_value"=>$display['section_slidesshow_active'] , "attached_options" => $display_boolean ),	
...
<script type="text/javascript">
	...
	$(document).ready(function(e) {
        var choose_id = $('#section_slidesshow_flag').val();
		if(choose_id == 1 || choose_id == 2) {
			$('#fieldset_section_slidesshow_url').show();
		} else {
			$('#fieldset_section_slidesshow_url').hide();
		}
		
		$('#section_slidesshow_flag').change(function(e) {
...
//Filters 
if($section_filter)
$section_string = " and section_slidesshow_section_id = ".$section_filter." ";
...
//Table Header
	...
	(
		...
		array("ID"=>"section_slidesshow_section_id", "title" => "Section" , "url_sort" => $this_page_name_with_varibles , "order" => "section_slidesshow_section_id" ,"type"=> "select" , "attached_options" => $display_section),
		array("ID"=>"section_slidesshow_active" , "title" => "Active" , "url_sort" => $this_page_name_with_varibles , "order" => "section_slidesshow_active" , "type"=> "text"),
...
$filter_options= array(
	...
	$additional_options = array(
		array("title" => "Active" , "url" => "{$this_page_name_with_varibles}", "additional_attr" => "section_slidesshow_active" , "pass_varible" => "" , "class_name"=>"approve_toggle" ,"icon" => "icon-3"),
	...
	{
		$data[$i]['ID'] = $display[$i]['section_slidesshow_ID'];	 
		$data[$i]['section_slidesshow_image_ar'] = $display[$i]['section_slidesshow_image_ar'];
		$data[$i]['section_slidesshow_image'] = $display[$i]['section_slidesshow_image'];
		$data[$i]['section_slidesshow_section_id'] = $display[$i]['section_slidesshow_section_id'];	
		$data[$i]['section_slidesshow_active'] = $display[$i]['section_slidesshow_active'] == 0 ? "No" : "Yes"; 
	}
