...
if($_POST['submit'] == "Add" ||  $_POST['submit'] == "Edit" )
{
	...
	if($_POST['submit'] == "Add" )
	{
		//chech if inseason_recipies_current_flag == 1
		if($tobesaved['inseason_recipies_current_flag'] == 1){
				$db->query("update {$this_table_name} set {$this_table_name}_current_flag = '0'");
			}
...
(
	...
	array("database_name" => "inseason_recipies_brief" , "title" => "Berif (English)" , "type" => "textarea" , "required" => "0" ,"current_value"=> $display['inseason_recipies_brief'] ) ,
	array("database_name" => "inseason_recipies_brief_ar" , "title" => "Berif (Arabic)" , "type" => "textarea" , "required" => "0" ,"current_value"=> $display['inseason_recipies_brief_ar'] ) ,
	...
	array("database_name" => "inseason_recipies_current_flag" , "title" => "Current Season" , "type" => "select" , "required" => "0" , "current_value"=>$display['inseason_recipies_current_flag'] , "attached_options" => $display_boolean ),	
...
$display = $db->querySelect("select * from $this_table_name {$join} where 1 and inseason_recipies_isnormaltopic = 0 {$search_query_string} {$sort_by_query} {$paging_query} ");
...
//Table Header
	...
	(
		...
		array("ID"=>"inseason_recipies_current_flag" , "title" => "Current Season" , "url_sort" => $this_page_name_with_varibles , "order" => "inseason_recipies_current_flag" , "type"=> "text"),
	);
	...
	array("title" => "Current Season" , "url" => "{$this_page_name_with_varibles}", "additional_attr" => "inseason_recipies_current_flag" , "pass_varible" => "" , "class_name"=>"approve_toggle" ,"icon" => "icon-3"),
	...
	for($i=0;$i<sizeof($display);$i++)
	{
		...
		$data[$i]['inseason_recipies_current_flag'] = $display[$i]['inseason_recipies_current_flag'] == 0 ? "No" : "Yes";
