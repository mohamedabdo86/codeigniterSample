...
$search_headers_attr = "areas_name;areas_name_ar"; //Seperate with semicolumns;

//handle Filters
$regions_filter = (int) $_GET['governments'];
...
$display_regions = $db->querySelect("select governments_ID as id,governments_name as value from  governments order by governments_name");
...
$form_array = array
(
	array("database_name" => "areas_name" , "title" => "Title (English)" , "type" => "text" , "required" => "1" ,"current_value"=> $display['areas_name'] ) ,
	array("database_name" => "areas_name_ar" , "title" => "Title (Arabic)" , "type" => "text" , "required" => "1" ,"current_value"=> $display['areas_name_ar'] , "isarabic" => true  ) ,
	array("database_name" => "areas_governments_ID" , "title" => "Government" , "type" => "select" ,"attached_options" => $display_regions, "required" => "1" ,"current_value"=> $display['areas_governments_ID']   ) ,
...
//Filters 
if ( $regions_filter != "" )
$regions_filter_string = " and areas_governments_ID=".$regions_filter;
...
//Table Header
	$table_header = array
	(
		array("ID"=>"areas_name", "title" => "Title (English)" , "url_sort" => $this_page_name_with_varibles , "order" => "governments_name" ,"type"=> "text"),
		array("ID"=>"areas_name_ar","title" => "Title (Arabic)" , "url_sort" => $this_page_name_with_varibles , "order" => "governments_name_ar", "type"=> "text" , "image_dir"=> $current_upload_directory ,"image_version" => "thumb" ) ,
		array("ID"=>"areas_governments_ID", "title" => "Government" , "url_sort" => $this_page_name_with_varibles , "order" => "governments_regions_ID" ,"type"=> "select" , "attached_options" => $display_regions),
	...
	for($i=0;$i<sizeof($display);$i++)
	{
		...
		$data[$i]['ID'] = $display[$i]['governments_ID'];
		$data[$i]['areas_name'] = $display[$i]['areas_name'];		
		$data[$i]['areas_name_ar'] = $display[$i]['areas_name_ar'];
		$data[$i]['areas_governments_ID'] = $display[$i]['areas_governments_ID'];
