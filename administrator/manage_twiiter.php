...
$order = $_GET['order']!="" ? $_GET['order'] : $this_table_name."_date";
...
$(document).ready(function(e) {
 ...
 /*   $('.approve_toggle').click(function(e) {*/
		...
		var id = $(this).data("id");
		var this_table_name = "<?php echo $this_table_name; ?>";
		var column_required_to_toggle = "tweet_approved";
		...
		$.ajax({
			  ...
			  success: function(success_array)
			  {
				   ...
				   $(".displaytables tr#"+id+"  td."+column_required_to_toggle+"" ).css("color","green").html(success_array.returned_array.members_issuspended ==0 ? "No" : "Yes");
...
if($bulk_actions_handler == "hide_all")
{
	$bulk_actions_items = $_POST['items_checkbox'];
	for($i=0;$i<sizeof($bulk_actions_items);$i++):
	$db->query("update {$this_table_name} set tweet_approved = 0 where {$this_table_name}_ID =".$bulk_actions_items[$i]);
	endfor;
	
	
	echo "<script type='text/javascript'>location.href='{$this_page_name_with_varibles}&state=updated'</script>";
	
}
if($bulk_actions_handler == "show_all")
{
	$bulk_actions_items = $_POST['items_checkbox'];
	for($i=0;$i<sizeof($bulk_actions_items);$i++):
	$db->query("update {$this_table_name} set tweet_approved = 1 where {$this_table_name}_ID =".$bulk_actions_items[$i]);
	endfor;
...
//Table Header
	$table_header = array
	(
		array("ID"=>"tweet_account", "title" => "Name" , "url_sort" => $this_page_name_with_varibles , "order" => "tweet_account" ,"type"=> "text"),
		array("ID"=>"tweet_account_image","title" => "user image" , "url_sort" => $this_page_name_with_varibles , "order" => "", "type"=> "text" , "image_dir"=> $current_upload_directory ,"image_version" => "thumb" ),		
		array("ID"=>"tweet_text","title" => "Tweet" , "url_sort" => $this_page_name_with_varibles , "order" => "", "type"=> "text")	 ,
		array("ID"=>"tweet_date","title" => "Date" , "url_sort" => $this_page_name_with_varibles , "order" => "tweet_date", "type"=> "text")	 ,
		array("ID"=>"tweet_approved","title" => "Approved?" , "url_sort" => $this_page_name_with_varibles , "order" => "tweet_approved", "type"=> "bool")	 
	...
	array("title" => "Show/Hide Tweet" , "url" => "{$this_page_name_with_varibles}" ,  "class_name"=>"hide_toggle" )
		
	);
	
	$bulk_actions = array( 
	
	array("id"=>"hide_all" , "value" => "Hide Selected Records"),
	array("id"=>"show_all" , "value" => "Show Selected Records")
	 );
	...
	{
		$data[$i]['ID'] = $display[$i]['tweet_ID'];
		$data[$i]['tweet_tweetid'] = $display[$i]['tweet_tweetid'];
		
		$data[$i]['tweet_account'] = $display[$i]['tweet_account'];
		$data[$i]['tweet_account_image'] = '<img src="'.$display[$i]['tweet_account_image'].'" />';
		$data[$i]['tweet_text'] = $display[$i]['tweet_text'];
		$data[$i]['tweet_date'] = date("d m Y , H:i" ,strtotime($display[$i]['tweet_date']) );
		$data[$i]['tweet_approved'] = $display[$i]['tweet_approved'];
