...
(
	...
	array("database_name" => "quizes_featured" , "title" => "Featured" , "type" => "select" , "required" => "0" , "current_value"=>$display['quizes_featured'] , "attached_options" => $display_boolean ),
...
//Pagination Handle
$total_numbers_of_rows = $db->numRows("select * from  $this_table_name  where 1  and quizes_flag = 'quiz' {$search_query_string}  ");
...
$display = $db->querySelect("select * from  $this_table_name  where 1 and quizes_flag = 'quiz'  {$search_query_string}  {$sort_by_query} {$paging_query} ");
...
//Table Header
	...
	(
		...
		array("title" => "View Result" , "url" => "manage_quizes_result.php?", "pass_varible" => "fkey"),
	);
