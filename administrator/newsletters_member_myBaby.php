...
//Pagination Handle
$total_numbers_of_rows = $db->numRows("select * from  $this_table_name where 1 && `newsletter_members_newsletter_types_id` = '8' {$search_query_string} {$section_string}");
...
$display = $db->querySelect("select * from $this_table_name where 1 && `newsletter_members_newsletter_types_id` = '8' {$search_query_string} {$section_string}   {$sort_by_query} {$paging_query}");
...
//Table Header
	...
	(
		...
		array("ID"=>"child_age","title" => "Baby's Month" , "url_sort" => $this_page_name_with_varibles , "order" => "child_age", "type"=> "text" ,  ),
		...
		array("ID"=>"newsletter_members_baby_birthdate","title" => "Baby's Birthdate" , "url_sort" => $this_page_name_with_varibles , "order" => "newsletter_members_baby_birthdate", "type"=> "text" ),
...
//	);
	...
	$options = array (
				...
				"delete" => array ( "placeicon"=>$delete_item_flag && in_array("$delete_access_token", $current_array_of_actions) , "url" =>$this_page_name_with_varibles ),
								"sendEmail" => array ( "placeicon"=>$delete_item_flag && in_array("$delete_access_token", $current_array_of_actions) , "url" =>$this_page_name_with_varibles ),
...
//	);
	$display_Age = array();
	for ($i=0;$i<=24;$i++){
		$display_Age[] = array('id'=>$i,'value'=>$i);
		
	}
	//print_r($display_Age);
	$filter_options= array(
		array("ID"=>"child_age","title" => "Child Age" ,"type" => "select" , "attached_options" =>$display_Age),
	...
	{		
		$display_type = $db->querySelectSingle("select * from newsletter_types where newsletter_types_ID = 8");
		...
		$data[$i]['child_age'] = $display[$i]['child_age'];
	...
	$export_Link = "<a style='float:right' href='scripts/newsletter_member_export.php";
	
	if($child_age)
	$export_Link .= "?child_age=".$child_age;
	
	$export_Link .="'>Export to Excel Sheet</a>";
	$generator->display_table($table_header,$data,$options,$export_Link,$filter_options);
