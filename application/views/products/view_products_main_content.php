...
<div class="related_3_items_list_wrapper <?php echo $current_section_color ?>">
            ...
            <ul class="bslider">
                  <?php
				  //Display subproducts
				  $count = sizeof($display_subproduct);
			
				  $jump = 0;
				  if($count > 3){
					  if($brand_id == 56){
						  $jump = ceil(($count/3)-1);
					  }elseif(($brand_id == 36) || ($brand_id == 121)){
						 $jump = ceil(($count/3) + 3); 
				  }else{
						 $jump = ceil(($count/3) + 1);
					   }
				  ...
				  $url = site_url("products/product_details/".$brand_id."/".$id);
				  ?>
                  <li>
                	<div class="panel">
                    	<a href="<?php echo $url; ?>" title="<?php echo $title; ?>">
                    	<h3 title="<?php echo $title; ?>" class="product_2nd_color"><?php echo $title; ?></h3>
                        <div title="<?php echo $title; ?>" class="image_wrapper product_background_color" style="border:none;background-image:url(<?php echo $image; ?>);"><img style=" display:none;border:none" src="" id="panel_image" alt="<?php echo $title; ?>" /></div>
                        <div id="panel_summary">
                        <?php /*?><a href="<?php echo site_url("products/product_details"); ?>"><label>لنمو صحى لخلايا العقل</label></a>
                        <p><span>من بعد سن الثالثة</span></p><?php */?>
                        <p id="paragraph">
                        <?php
						if($short_desc){
						 echo $short_desc;
