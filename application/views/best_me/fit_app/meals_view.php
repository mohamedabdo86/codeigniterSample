		  ...
		  source: function( request, response ) {
		   ...
		   select: function( event, ui ) 
		   {
			   var current_id = $(this).attr('id');
				...
				//$('#InputsWrapper_<?php echo $id; ?> li #tname_'+current_id+'').val(ui.item.tname);
		...
		if($array_lenght != 0) {
			for($i=0;$i<$array_lenght;$i++) {
				...
				//if($tname == "recipes") {
					...
					$meals = $array[$i]['recipes_title'.$current_language_db_prefix];
					...
					$calory = $array[$i]['recipes_calories'];
					
				//} elseif ($tname == "nestle_fit_food") {
//					
//					$meals = $array[$i]['nestle_fit_food_title'.$current_language_db_prefix];
//					$amount = $array[$i]['nestle_fit_meals_amount'];
//					$value = $array[$i]['nestle_fit_meals_meal'];
//					$type = $array[$i]['nestle_fit_measurments_title'.$current_language_db_prefix];
//					$calory = $array[$i]['nestle_fit_food_calories'];
//					
//				}
				...
				$display_string .= '<input type="hidden" name="calory_value" value="'.$calory.'" id="calory_'.($i+1).'" />';
				$display_string .= '<input type="hidden" name="calories" value="'.$calory.'"/>';
				$display_string .= '<input type="hidden" name="measurments" value="'.$type.'"/>';
				
				// table of meal "nestle_fit_food OR recipes"
				//$display_string .= '<input type="hidden" name="tname[]" value="'.$tname.'" id="tname_'.($i+1).'"/>';
				
				$display_string .= '<div class="float_left meal-calories" data-total="'.$calory*$amount.'" style="width: 150px;">'.$calory*$amount." ".lang('Calories_day').'</div>';
	...
	?>
		<script type="text/javascript">
            $(document).ready(function(e) {
                $('.meal_recipe').prop( "disabled", true );
                $('.meal_amount').prop( "disabled", true );
            });
        </script>
    <?php
	echo $display_string;
...
//					$calory = $array[$i]['nestle_fit_food_calories'];
				...
				$display_string .= '<input type="text" name="the_meal[]" placeholder="'. lang('globals_meal').'" id="'.($i+1).'" value="'.$meals.'" class="meal_recipe search float_left" />';
				$display_string .= '<input type="text" name="meal_amount[]" placeholder="'. lang('globals_amount').'" id="amount_'.($i+1).'" value="'.$amount.'" class="meal_amount float_left" />';
				$display_string .= '<div class="float_left mealmeasure" id="measure'.($i+1).'" >'.$type.'</div>';
				$display_string .= '<input type="hidden" name="meal_value[]" class="meal_value" value="'.$value.'" id="field_'.($i+1).'" />';
				$display_string .='<input type="hidden" name="calory_value" value="'.$calory.'" id="calory_'.($i+1).'" />';
				
				$display_string .= '<input type="hidden" name="calories" value="'.$calory.'"/>';
				$display_string .= '<div class="float_left meal-calories" data-total="'.$calory*$amount.'" style="color: white; width: 150px;">'.$calory*$amount." ".lang('Calories_day').'</div>';
