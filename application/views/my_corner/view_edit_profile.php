...
$(function () {
    ...
    $('#fileupload').fileupload({
			...
			if (uploadFile.size > 5000000) { // 5mb
				//common.notifyError('Please upload a smaller image, max size is 5 MB');
...
$(document).ready(function(e) {
	...
	$("#update_order").fancybox({
		  ...
		  afterLoad: function(){
   			setTimeout( function() {$.fancybox.close(); },5000); // 3000 = 3 secs
  		}
      });
...
#email_message , #username_message , #mobile_message
{
	position: absolute;
	left: 442px;
	margin-top: 10px;
}
#email_message p , #username_message p , #mobile_message p
{
	width:310px;
}
body.arabic #username_message , body.arabic #email_message , body.arabic #mobile_message
{
	position: absolute;
	left: 316px;
	margin-top: 10px;
...
$username = $members_info[0]['members_nickname'];
...
<h1 style="display:inline;" class="<?php echo $current_section_color ?>"><a class="edit_button" id="info" href="<?php echo site_url($this->router->class."/profile") ?>" ><?php echo lang('globals_hmenu_myprofile'); ?></a></h1>
</div>
...
<div class="profile_container global_background " style="background-image:url('<?php //echo base_url()."images/mycorner/profile_bg.jpg"; ?>')">
    ...
    <div id="first_row">
      ...
      <table width="100%" border="0" cellpadding="5" class="float_left direction">
		<tr>
        	<td width="25%"><h5><?php echo lang('mycorner_firstname');?> <span style="color:red"> * </span></h5></td>
        	<td width="65%">
			<?php 
			$data=array( 'name' => 'members_first_name' , 'id' => 'members_first_name','class'=>'date_text', 'value' => $first_name, 'size' => 50, 'onkeypress'=>'return onlyAlphabets(event,this);');                  
            echo form_input($data);	
        ...
        <tr>        
            <td><h5><?php echo lang('mycorner_lastname');?> <span style="color:red"> * </span></h5></td>
       		<td>
         	<?php 
			$data=array( 'name' => 'members_last_name', 'id' => 'members_last_name' ,'class'=>'date_text' ,'value' => $last_name, 'size' => 50,'onkeypress'=>'return onlyAlphabets(event,this);'); 			 
			echo form_input($data);
        ...
        <tr><td><a class="fancybox fancybox.ajax" style="font-size: 14px;padding: 5px;color:#666;line-height: 33px;" href="<?php echo site_url("my_corner/change_your_password"); ?>"><?php echo lang('mycorner_change_your_password'); ?></a></td></tr>
        <tr>
        <td>
        ...
        <td>
            <div id="status_image_wrapper" align="left" style="float: left;">
        ...
        <div class="float_left m_10_5" style=" width:600px; ">
          <?php 
		   ...
		   //email_flag
			...
			//username_flag
            $data=array('type'=>'hidden' ,'name' => 'username_flag' ,'value' => 1 , 'id' => 'username_flag'); 		 
			echo form_input($data);
			
			//current_email
            $data=array('type'=>'hidden' ,'name' => 'current_username' ,'value' => $username , 'id' => 'current_username'); 		 
			echo form_input($data);			
		
			
		   ?>
      <table width="105%" border="0" class="float_left direction">
		<tr>
        ...
        <tr>
        	<td><h5><?php echo lang('mycorner_username');?><span style="color:red"> * </span></h5></td>
            <td>
            <?php
              	$data=array( 'name' => 'username' ,'id' => 'username','AUTOCOMPLETE'=>'off','class'=>'small_text', 'value' => $username ,'onkeypress'=>'return onlyenglishandNumber(event);');                  
              	echo form_input($data);
				echo form_error('username');
				echo '<img class="availability_username" src="'.base_url().'images/camera-loader.gif" />';
				
			  //	echo '<p class="field_error red_color">'. form_error('username') .'</p>';
				echo '</td><td id="username_message">';
			  	
			  	echo '<p id="username_error" class="field_error red_color">'. lang("bestcook_field_required").'</p>';
				echo '<p id="username_error2" class="field_error red_color">'.lang("mycorner_caution_username_min").'</p>';
			  	echo '<p id="members_username_validation" class="field_error red_color"></p>'; 
			  	echo '<p id="members_username_format_error" class="field_error red_color">'.lang('mycorner_unavailable_username').'</p>';
			  ?>
            </td>
        ...
        <tr>
            ...
            <td>
			 ...
			 <?php 
                ...
                $data=array( 'name' => 'members_email' ,  'id' => 'members_email' ,'class'=>'small_text float_left' , 'value' => $email); 			 
                echo form_input($data);
                if(!$current_language_db_prefix == "_ar")
                {
                    echo '<img class="availability_email" src="'.base_url().'images/camera-loader.gif" />';
             ...
             echo '</td><td id="email_message">';
					echo form_error('members_email'); 
					echo '<p id="members_email_error" class="field_error red_color">'. lang("bestcook_field_required").'</p>';
					echo '<p id="members_email_validation" class="field_error red_color"></p>'; 
					echo '<p id="members_email_format_error" class="field_error red_color">'.lang('globals_lform_not_vaild_format').'</p>'; 
			...
			<td>
             	<?php 
			 	$data=array( 'name' => 'members_mobile', 'id' => 'members_mobile' ,'maxlength' => '11' ,'class' => 'small_text','value' => $mobile , 'size' => 50 ,'onkeypress'=>'return isNumberKey(event)'); 
			  	echo form_input($data);	 
				echo form_error('members_mobile');
			 	echo "</td><td id='mobile_message'><p class='members_mobile_error field_error'>".lang("bestcook_field_required")."</p>" 	  
				?>
            </td>
        </tr>
...
$(document).ready(function(e) {
    ...
    <div id="fourth_row">
      ...
      <table width="100%" border="0" class="float_left direction">
		  ...
		  echo '<td style="width:100px;">'.lang('mycorner_pregnancy_month').'</td>';
            ...
            <div class="float_left">
				...
				if($members_section_order)
				{
					for($i=0;$i<sizeof($members_section_order);$i++)
					{
						...
						echo '<li id='.$section_id.'" class="ui-state"><div class=" section_order '.$section_title.'_background_color"><h3>'.$section_name.'</h3></div></li>';
					}
				}
				else
				{?>
