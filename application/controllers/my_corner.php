   ...
   public function __construct()
   {
	...
	public function signup_success($members_id)
	{
		...
		if($this->input->post('update') == true)
		{
			...
			list($current_image_width, $current_image_height) = getimagesize('./uploads/members/'.$uploaded_image);
			if($current_image_width > 470)
			{
				$this->resizeclass->fit_to_width(470);
				$this->resizeclass->saveImage('./uploads/members/'.$uploaded_image, 80);						
			}
		...
		$type = extractSeoid($type);
		 ...
		 if($this->input->post('update') == true)
		 {
			...
			if($this->input->post('edit_type') == 'info')
			{
				...
				$this->form_validation->set_rules("members_first_name", lang('mycorner_name'), "trim|required|min_length[2]|max_length[20]|callback_custom_server_side_firstlastname_form_validator[members_first_name]");
				$this->form_validation->set_rules("members_last_name", lang('mycorner_email'), "trim|required|min_length[2]|max_length[20]|callback_custom_server_side_firstlastname_form_validator[members_last_name]");
			...
			else if($this->input->post('edit_type') == 'address')
			{
				...
				$this->form_validation->set_rules("username", lang('mycorner_name'), "trim|required|min_length[8]|max_length[20]|callback_custom_server_side_username_form_validator[username]");
	...
	{
		$this->load->view("my_corner/view_send_activation_mail", $this->data);
		//$this->load->view("my_corner/view_forgot_your_password", $this->data);
	}
	...
	///////////////////////// send another mail activation////////////////
	public function send_another_mail_activation()
	{
		$this->load->library('emailmanager');
		$email = $this->input->post('forgot_your_password_email');
		$code_generateed =  md5(uniqid(rand(), true));
		...
		{
		 $array_of_status['status'] = 0;
		}else if($display[0]['members_activated']==1){
				$array_of_status['status'] = 2;	 
		}else{
			
         if ($display[0]['members_lang'] == "english"){
			$current_lang="en";
			}else{
			$current_lang="ar";	
				}
		$current_member_id =$display[0]['members_ID'];	
        $data['name'] = $display[0]['members_first_name']. ' '. $display[0]['members_last_name'];
        $data['url'] = site_url($current_lang."/my_corner/validate_account/" . $current_member_id . "/" . $display[0]['members_activated']);
         
        if ($display[0]['members_lang'] == "english")
        {
            $send =  $this->emailmanager->send_email($display[0]['members_email'], "Activation Email", $data, 'email_activation_en');
        }
        else
        {
            $send = $this->emailmanager->send_email($display[0]['members_email'], "Activation Email", $data, 'email_activation');
        }
			if($send){
				$array_of_status['status'] = 1;
			}else{
	...
	public function delete_user_favourites($id){
		...
		$delete_query = $this->membersmodel->delete_user_favourites($id);
		echo $delete_query;
	}
